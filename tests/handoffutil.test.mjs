...
console.log('— §13 Wave A · A1: City / State — hardened parser (prefer blank over wrong) + structured fields —');
{
  ...
  const cases = [
    ...
    ['Unit 2, 5 Palm La 70801',                               {}, 'pass 2→4: "Palm La" is Palm Lane — a Title-case suffix code is never a state, even with an agreeing ZIP → blank'],
    ...
    // pass 3 (verification refuter): geocoder shapes with a county segment, USPS Pub 28 designators, doubled tokens
    ['412 Oak St, Charlotte, Mecklenburg County, North Carolina, 28202, United States', { city: 'Charlotte', state: 'NC' }, 'pass 3: OpenStreetMap / Nominatim shape — the county steps back to the city, never ships as the city'],
    ['100 Ocean Blvd, Myrtle Beach, Horry County, South Carolina, 29577, United States', { city: 'Myrtle Beach', state: 'SC' }, 'pass 3: same, two-word city'],
    ['55 Main St, Media, Delaware County, Pennsylvania, 19063, United States', { city: 'Media', state: 'PA' }, 'pass 3: "Delaware County" is neither a city nor a state'],
    ['412 Oak St, Charlotte, Mecklenburg County, NC 28202',    { city: 'Charlotte', state: 'NC' }, 'pass 3: short county form, whole-segment code'],
    ['412 Oak St, Charlotte, Mecklenburg County NC 28202',     { city: 'Charlotte', state: 'NC' }, 'pass 3: county inside the state segment'],
    ['412 Oak St, Mecklenburg County, NC 28202',               { state: 'NC' }, 'pass 3: county with no city before it → state only (the street line is never a city)'],
    ['412 Oak St, Rear, NC 28202',                             { state: 'NC' }, 'pass 3: "Rear" is a USPS secondary designator, never a city'],
    ['412 Oak St, Basement, NC 28202',                         { state: 'NC' }, 'pass 3: "Basement" likewise'],
    ['412 Oak St, Penthouse, NC 28202',                        { state: 'NC' }, 'pass 3: "Penthouse" likewise'],
    ['412 Oak St, Attn Jane Doe, NC 28202',                    { state: 'NC' }, 'pass 3: an attention line is never a city'],
    ['412 Oak St, Charlotte, NC, NC 28202',                    { state: 'NC' }, 'pass 3: a doubled state token is never the city (state only, never city "NC")'],
    ['412 Oak St, Charlotte, North Carolina, 28202, United States', { city: 'Charlotte', state: 'NC' }, 'pass 3: control — ZIP as its own segment, country suffix'],
    // pass 3 (critic): the secondary rule must be anchored to USPS shapes — real cities that START with a designator word survive
    ['1 Main St, Key West, FL 33040',                          { city: 'Key West', state: 'FL' }, 'pass 3: "Key West" is a city, not "Key <identifier>"'],
    ['1 Main St, Key Largo, FL 33037',                         { city: 'Key Largo', state: 'FL' }, 'pass 3: Key Largo'],
    ['1 Main St, Front Royal, VA 22630',                       { city: 'Front Royal', state: 'VA' }, 'pass 3: Front Royal'],
    ['1 Main St, Upper Arlington, OH 43221',                   { city: 'Upper Arlington', state: 'OH' }, 'pass 3: Upper Arlington'],
    ['1 Main St, Lower Burrell, PA 15068',                     { city: 'Lower Burrell', state: 'PA' }, 'pass 3: Lower Burrell'],
    ['1 Main St, Ste. Genevieve, MO 63670',                    { city: 'Ste. Genevieve', state: 'MO' }, 'pass 3: "Ste." followed by a name is a city, not a suite'],
    ['1 Main St, Ste 400, MO 63670',                           { state: 'MO' }, 'pass 3: "Ste 400" is a suite → state only'],
    ['1 Main St, Apt. 4B, MO 63670',                           { state: 'MO' }, 'pass 3: dotted designator + identifier'],
    ['1 Main St, Lot 7, MO 63670',                             { state: 'MO' }, 'pass 3: "Lot 7"'],
    ['1 Main St, #4, MO 63670',                                { state: 'MO' }, 'pass 3: "#4"'],
    ['1 Main St, Key Biscayne, FL 33149',                      { city: 'Key Biscayne', state: 'FL' }, 'pass 3/4: a designator word followed by a NAME is not a designator (real city, real ZIP)'],
    ['1 Main St, Houston, TX 77058',                           { city: 'Houston', state: 'TX' }, 'pass 4: control — a real Houston ZIP'],
    ['1 Foo Rd, Main St NE 68102',                             { state: 'NE' }, 'pass 3: a street line beside an agreeing-ZIP state is never the city → state only'],
    ['1 Foo Rd, Elm Ave, NC 28202',                            { state: 'NC' }, 'pass 3: same, whole-segment state'],
    ['1234 Farnam St, Omaha NE',                               {}, 'pass 3: DOCUMENTED LIMIT — Nebraska in the "City NE" shape with no ZIP stays blank (NE reads as a directional)'],
    ['1234 Farnam St, Omaha, NE',                              { city: 'Omaha', state: 'NE' }, 'pass 3: …the comma form parses'],
    ['1234 Farnam St, Omaha NE 68102',                         { city: 'Omaha', state: 'NE' }, 'pass 3: …and so does the ZIP form'],
    ['10 W Cherry St, Nevada, MO 64772',                       { city: 'Nevada', state: 'MO' }, 'pass 3: a state-name city with a comma before its state is fine'],
    ['1 Main St, Washington, DC 56901',                        { city: 'Washington', state: 'DC' }, 'pass 3/4: an unknown ZIP prefix (569, a DC parcel-return range not in the table) neither vouches nor vetoes — the whole-segment rule decides (coherent fixture)'],
    ['1 Main St, Washington DC 56901',                         {}, 'pass 4: …but the IN-SEGMENT form is refused with an unknown prefix (documented: only the whole-segment form is accepted then)'],
    ['1 Main St, Tamuning, GU 96913',                          {}, 'pass 4: Guam is not a state → blank'],
    ['1 Main St, Charlotte Amalie, VI 00801',                  {}, 'pass 3: VI is not a state → blank'],
    ['1 Main St, Conway, SC 29526',                            { city: 'Conway', state: 'SC' }, 'pass 3: "way" inside a word is not a street suffix'],
    ['1 Main St, Broadway, VA 22815',                          { city: 'Broadway', state: 'VA' }, 'pass 3: Broadway likewise'],
    ['1 Main St, Unity, ME 04988',                             { city: 'Unity', state: 'ME' }, 'pass 3: "Unity" is a city — a designator needs a space before its identifier'],
    ['1 Main St, Unit 4, ME 04988',                            { state: 'ME' }, 'pass 3: "Unit 4" is a designator'],
    ['1 Main St, Reardan, WA 99029',                           { city: 'Reardan', state: 'WA' }, 'pass 3/4: a bare designator must be the WHOLE word ("Rear" ≠ "Reardan"; real city, real ZIP)'],
    // pass 4 (verification refuters): street lines with spelled-out suffixes / directionals, mail lines, Title-case suffix codes, West Virginia, pasted separators
    ['1 Foo Rd, Elm Avenue, NC 28202',                         { state: 'NC' }, 'pass 4: a spelled-out street suffix is a street line → state only'],
    ['1 Foo Rd, Oak Boulevard, NC 28202',                      { state: 'NC' }, 'pass 4: same'],
    ['1 Foo Rd, Main St NE, GA 30309',                         { state: 'GA' }, 'pass 4: a trailing directional does not rescue a street line'],
    ['Apt 5, M St SW, DC 20024',                               { state: 'DC' }, 'pass 4: same, DC quadrant'],
    ['Lot 12, Pine Ridge Trail, NC 28712',                     { state: 'NC' }, 'pass 4: "Trail" is a suffix'],
    ['APT 5, PEACHTREE STREET NE, GA 30309',                   { state: 'GA' }, 'pass 4: all-caps street line beside a whole-segment state + agreeing ZIP → state only'],
    ['1 Main St, General Delivery, NC 28202',                  { state: 'NC' }, 'pass 4: a mail-handling line is never a city'],
    ['1 Main St, Rural Route, NC 28202',                       { state: 'NC' }, 'pass 4: same'],
    ['12 Elm Rd, Oak Ct 06604',                                {}, 'pass 4: a Title-case "Ct" is Court, never Connecticut — even beside an agreeing ZIP → blank, never Oak / CT'],
    ['12 Elm Rd, Palm La 70801',                               {}, 'pass 4: Title-case "La" is Lane → blank'],
    ['12 Orange St, Bridgeport ct 06604',                      { city: 'Bridgeport', state: 'CT' }, 'pass 4: a code typed lower-case beside an agreeing ZIP still parses (only the Title-case street-word shape is refused)'],
    ['1 Main St, Charleston West Virginia 25301',              { city: 'Charleston', state: 'WV' }, 'pass 4: "Virginia" contradicts the WV ZIP, so the loop keeps looking and finds "West Virginia"'],
    ['412 Oak St, Charlotte, NC 28202,',                       { city: 'Charlotte', state: 'NC' }, 'pass 4: a trailing comma is noise'],
    ['412 Oak St\nCharlotte\nNC 28202',                        { city: 'Charlotte', state: 'NC' }, 'pass 4: newlines are separators (mobile multi-line paste)'],
    ['412 Oak St; Charlotte; NC 28202',                        { city: 'Charlotte', state: 'NC' }, 'pass 4: semicolons likewise'],
    ['412 Oak St, Charlotte, NC 282021234',                    { city: 'Charlotte', state: 'NC' }, 'pass 4: a 9-digit ZIP without the hyphen'],
    ['1 Main St, Rockville Centre, NY 11570',                  { city: 'Rockville Centre', state: 'NY' }, 'pass 4: "Centre" is not a street suffix here'],
    ['1 Main St, Circle, AK 99733',                            { city: 'Circle', state: 'AK' }, 'pass 5: Circle, Alaska is a city — a place-name word without a directional is not a street line'],
    // pass 5 (verification refuters): spelled-out directionals; place-name suffix words stay cities
    ['Peachtree Street Northeast, Fulton County, Georgia, 30309, United States', { state: 'GA' }, 'pass 5: a street line with a SPELLED-OUT directional (OSM shape, no city) → state only, never the street as the city'],
    ['Ocean Drive South, Horry County, South Carolina, 29575, United States', { state: 'SC' }, 'pass 5: same'],
    ['Lake Shore Drive North, Cook County, Illinois, 60611',   { state: 'IL' }, 'pass 5: same'],
    ['1 Foo Rd, Elm Avenue Northeast, NC 28202',               { state: 'NC' }, 'pass 5: same, comma form'],
    ['1 Foo Rd, Main St North, NC 28202',                      { state: 'NC' }, 'pass 5: abbreviated suffix + spelled-out directional'],
    ['Peachtree Street Northeast, Atlanta, Fulton County, Georgia, 30309', { city: 'Atlanta', state: 'GA' }, 'pass 5: control — with the city present it is the city'],
    ['412 Oak St, Federal Way, WA 98003',                      { city: 'Federal Way', state: 'WA' }, 'pass 5: Federal Way is a city (place-name suffix word, no directional)'],
    ['412 Oak St, Temple Terrace, FL 33617',                   { city: 'Temple Terrace', state: 'FL' }, 'pass 5: Temple Terrace'],
    ['412 Oak St, Franklin Square, NY 11010',                  { city: 'Franklin Square', state: 'NY' }, 'pass 5: Franklin Square'],
    ['412 Oak St, College Place, WA 99324',                    { city: 'College Place', state: 'WA' }, 'pass 5: College Place'],
    ['412 Oak St, Green Lane, PA 18054',                       { city: 'Green Lane', state: 'PA' }, 'pass 5: Green Lane'],
    ['412 Oak St, Grand Terrace, CA 92313',                    { city: 'Grand Terrace', state: 'CA' }, 'pass 5: Grand Terrace'],
    ['1 Foo Rd, Ocean Way NE, NC 28202',                       { state: 'NC' }, 'pass 5: a place-name suffix word WITH a directional is a street line'],
    ['1 Foo Rd, Ocean Way Northeast, NC 28202',                { state: 'NC' }, 'pass 5: same, spelled out'],
    ['1 Foo Rd, Elm Ter, NC 28202',                            { state: 'NC' }, 'pass 5: an abbreviated place-name suffix is a street line'],
    // pass 6 (verification refuter): a spelled-out single-word state name needs a ZIP witness; USPS abbreviations
    ['1 Main St, Old Town, Nevada',                            {}, 'pass 6: "Nevada" as the last segment with no ZIP is as likely Nevada, MO → blank, never NV'],
    ['1 Main St, Riverside, Delaware',                         {}, 'pass 6: Delaware, OH → blank, never DE'],
    ['1 Main St, Uptown, California',                          {}, 'pass 6: California, PA / MD → blank, never CA (a referred-out state)'],
    ['1 Main St, Georgetown, Washington',                      {}, 'pass 6: Washington the city → blank, never WA'],
    ['1 Main St, Miami, Florida',                              {}, 'pass 6: DOCUMENTED LIMIT — a one-word state name with no ZIP blanks'],
    ['1 Main St, Miami Beach, Florida 33139',                  { city: 'Miami Beach', state: 'FL' }, 'pass 6: …an agreeing ZIP makes it confident (33139 is Miami Beach)'],
    ['1 Main St, Miami, FL',                                   { city: 'Miami', state: 'FL' }, 'pass 6: …and a code stands on its own'],
    ['6001 S Kings Hwy, Myrtle Beach, South Carolina',         { city: 'Myrtle Beach', state: 'SC' }, 'pass 6: a multi-word state name stands on its own without a ZIP'],
    ['100 Flatbush Ave, Brooklyn, New York',                   { city: 'Brooklyn', state: 'NY' }, 'pass 6: same, New York'],
    ['1 Main St, Charleston, West Virginia',                   { city: 'Charleston', state: 'WV' }, 'pass 6: same, West Virginia'],
    ['1 Foo Rd, Oak Wy, NC 28202',                             { state: 'NC' }, 'pass 6: "Wy" is the USPS abbreviation of Way → street line'],
    ['1 Foo Rd, Oak Xing, NC 28202',                           { state: 'NC' }, 'pass 6: "Xing" (Crossing) likewise'],
    ['Elm Trce, Mecklenburg County, North Carolina, 28202, United States', { state: 'NC' }, 'pass 6: an abbreviated street line reached through the county step-back → state only'],
    ['418, Oak Cove, Horry County, South Carolina, 29577, United States', { city: 'Oak Cove', state: 'SC' }, 'pass 6: DOCUMENTED ECHO — an unlisted street word in a city-less paste ships as the user\'s own text; the state is right'],
    ['1 Main St, Oak Ridge, TN 37830',                         { city: 'Oak Ridge', state: 'TN' }, 'pass 6: control — "Ridge" spelled out is a city word (Oak Ridge, Glen Cove, Grants Pass, Chevy Chase, Mountain View stay cities)'],
    ['1 Main St, Glen Cove, NY 11542',                         { city: 'Glen Cove', state: 'NY' }, 'pass 6: control'],
    ['1 Main St, Mountain View, CA 94040',                     { city: 'Mountain View', state: 'CA' }, 'pass 6: control'],
    // pass 7 (verification refuter + critic): Wy joins the suffix-code guard; the all-caps echo and the Title-case-only guard are pinned as law
    ['1 Foo Rd, Oak Wy 82001',                                 {}, 'pass 7: a Title-case "Wy" is Way, never Wyoming — even beside an agreeing ZIP → blank'],
    ['1 Foo Rd, OAK WY 82001',                                 { city: 'OAK', state: 'WY' }, 'pass 7: DOCUMENTED ECHO — all-caps cannot be told from a code typed as a code; the state is right'],
    ['12 ELM RD, OAK CT 06604',                                { city: 'OAK', state: 'CT' }, 'pass 7: DOCUMENTED ECHO — same, and the reason the guard is Title-case only:'],
    ['12 ORANGE ST, BRIDGEPORT CT 06604',                      { city: 'BRIDGEPORT', state: 'CT' }, 'pass 7: …a real all-caps MLS paste of the Orange Street shape must keep parsing'],
    ['418 Oak Wy, Cheyenne, WY 82001',                         { city: 'Cheyenne', state: 'WY' }, 'pass 7: control — a normal Wyoming address'],
    ['1 Main St, Trail, MN 56684',                             { state: 'MN' }, 'pass 7: DOCUMENTED LIMIT — a municipality whose whole name is a street word (Trail, Minnesota) → state only'],
    ['1 Main St, Highway, KY 41465',                           { state: 'KY' }, 'pass 7: same (Highway, Kentucky)'],
    ['412 OAK ST, CHARLOTTE NC',                               {}, 'pass 7: DOCUMENTED LIMIT — all-caps with an in-segment code and no ZIP → blank'],
    ['412 OAK ST, CHARLOTTE, NC',                              { city: 'CHARLOTTE', state: 'NC' }, 'pass 7: …the comma form parses'],
    ['1 Main St, Bonita Spgs, FL 34134',                       { state: 'FL' }, 'pass 7: a user-abbreviated city withholds the city (blank, never wrong)'],
    ['',                                                      {}, 'empty'],
    [null,                                                    {}, 'null'],
  ];
